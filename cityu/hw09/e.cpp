...
int main (){
    ...
    id[0]=id[p]; id[p+1]=id[1];
    int ans=0;
    for (int i=1;i<=p;i++){
        if (!(y[id[i-1]]>y[id[i]] && y[id[i+1]]>y[id[i]])) continue;
        int xid=id[i]+1, bid=id[i]-1;
        if (xid>n) xid-=n;
        if (bid<1) bid+=n;
        vec na(x[id[i]],y[id[i]]),nb(x[xid], y[xid]), nc(x[bid],y[bid]), vb=nb-na, vc=nc-na;
        // cout << " ? " << vb.x << ' '<< vb.y << " | " << vc.x << ' ' << vc.y << " = " << vb.cross(vc) << '\n';
        if (vb.cross(vc)>0) ans++;
    }
    cout << ans;
}
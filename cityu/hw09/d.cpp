...
pair<vec,vec> tang(vec c, db r, vec p){
    db b=p.dist(c), th=acosl(r/b), d=atan2l(p.y-c.y,p.x-c.x);
    db d1=d+th, d2=d-th;
    return {vec(c.x+r*cosl(d1), c.y+r*sinl(d1)), vec(c.x+r*cosl(d2), c.y+r*sinl(d2))};
}
...
int main (){
    ...
    if (b==-1) ans+=2.0*(n-1);
    if (f<n && f>0) {
        vec tp(0.5,sqrtl(3.0)*0.5);
        if (s[f]=='S'){
            vec sp(f,1.0);
            ans+=tp.dist(sp)+0.5;
        }
        else{
            auto [vc1,vc2]=tang(vec(f+0.5,0.5),0.5,tp);
            if (vc1.y<vc2.y) swap(vc1,vc2);
            db ang=(vc1-vec(f+0.5,0.5)).angle() - PI/2;
            ans+=tp.dist(vc1) + PI*(ang)/(2*PI);
        }
        ans+=f;
    }
    if (b<n-1 && b>=0) {
        vec tp(db(n)-0.5,sqrtl(3.0)*0.5);
        if (s[b]=='S'){
            vec sp(b+1,1.0);
            ans+=tp.dist(sp)+0.5;
        }
        else{
            auto [vc1,vc2]=tang(vec(b+0.5,0.5),0.5,tp);
            if (vc1.y<vc2.y) swap(vc1,vc2);
            db ang=PI/2 - (vc1-vec(b+0.5,0.5)).angle();
            // cout << " ! " << vc1.x << ' ' << vc1.y << ' ' << ang/2 << '\n';
            ans+=tp.dist(vc1) + PI*(ang)/(2*PI);
        }
        ans+=n-1-b;
    }
